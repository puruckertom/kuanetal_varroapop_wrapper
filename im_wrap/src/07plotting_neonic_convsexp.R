...
for (n in 1:nrows){
  ...
  z <- which(tdarray_level3[n, 6, 1:Nsims] > 50)
  zz <- which(tdarray_level2[n, 6, 1:Nsims] > 50)
  cwb_control[n] <- length(x)/Nsims
  cwb_level1[n] <- length(y)/Nsims
  cwb_level3[n] <- length(z)/Nsims
  cwb_level2[n] <- length(zz)/Nsims
}
...
pdf(file= paste(vpdir_fig, "fig_1_MCproportions_convsexp.pdf", sep=""), width = 5, height = 9, onefile = TRUE, paper = "USr")
  ...
  lines(timearray, cdb_level3, type="l", lty = 2, col="black")
  lines(timearray, cdb_level2, type="l", lty = 2, col="green")
...
for (r in 1:5){
  for (t in 1:nrows){
    ...
    q<- quantile(temparray_level3[t, r, 1:Nsims])
    n<- quantile(temparray_level2[t,r,1:Nsims])
    for (s in 1:3){
      quant_control<- c(p[[2]], p[[3]], p[[4]])
      quant_level1<- c(z[[2]], z[[3]], z[[4]])
      quant_level3<- c(q[[2]],q[[3]],q[[4]])
      quant_level2<- c(n[[2]],n[[3]],n[[4]])
