...
#immmites<- runif(Nsims, 0, 200)
#im2<- runif(Nsims, 0, 200)
...
sb1_input<- data.frame(weather_file, sim_start, sim_end, foliar_true,foliar_appdate,foliar_begin,
                       foliar_end,drnadults,wkradults,drnbrood,wkrbrood,drnlarv,wkrlarv,drneggs,
                       wkreggs,queenstrength,fgrlifespan,wkrdrnratio,adslope,adLD50,adslopec,
                       adLD50c,lslope,lLD50,kow,koc,halflife,apprate, stringsAsFactors = FALSE)
sb2_input<- data.frame(weather_file,sim_start,sim_end,foliar_true,foliar_appdate,foliar_begin,
                       foliar_end,da2,wa2,db2,wb2,dl2,wl2,de2,we2,qs2,fl2,wd2,as2,aL2,asc2,aLc2,
                       ls2,lL2,kow2,koc2,hl2,ar2, stringsAsFactors = FALSE)
...
sb_inputs <- array(data = NA, c(Nsims, Nparam, Nparam-6))
sb_inputs[,,1] <- abind(sb1_input, along = 3)
sb_inputs[,,2] <- abind(sb2_input, along = 3)
for (n in 8:Nparam){
  a <- format(sb1_input, scientific=FALSE)
  a[,n]<- format(sb2_input[n], scientific=FALSE)
  assign(paste("sb",n-6,"_input", sep=""), a)
  sb_inputs[,,n-6] <- abind(a, along = 3)
}
